 ...
 * 兼容性测试 - Dialog
 */
public class CommonDialogInfo extends DemoCommonUsageInfo {
    TestSheetDialog mSheetDialog;
    PopupDesc mPopupDesc;

    public CommonDialogInfo() {
        title = "Dialog中弹出BasePopup";
        option = "描述";
//        问题描述：在没有依赖BasePopup Compat组件时，在Fragment或者Dialog里弹出，会显示在这些组件下层。
    }
    ...
    public void toOption(View v) {
        if (mPopupDesc == null) {
            ...
            mPopupDesc.setTitle("Dialog中弹出BasePopup")
                      .setDesc(new StringBuilder("问题描述：")
                                       .append('\n')
                                       .append("在Dialog或Fragment中弹出BasePopup时，由于默认取Activity的DecorView的WindowToken的缘故，因此会默认显示在Dialog或Fragment的下层。")
                                       .append('\n')
    ...
    static class TestSheetDialog extends Dialog {
        TextView tvShow;
